# 제어문
# 1. 조건문: if~elif~else
# 2. 반복문 : for(반복 횟수가 정해져있을때 사용), 
#             while(반복 횟수가 정해져있지 않을때 사용)
...
# 리스트 컴프리헨션
'''
변수 = [수행문1 if 조건문 수행문2 for 변수 in 리스트]
'''

new_scores2 = [score + 5 if score < 100 else score for score in scores]

print('47:', new_scores2)

# while
# while일경우 조선식이 참일때 수행문을 무한히 반복함
'''
무한 루프
while 조건식 :   
    수행문
...
new_scores = []
index = 0
# crtl+d = 클릭수만큼 같은 단어 다중선택
while index < len(scores):
     if scores[index] < 100:
          new = scores[index] + 5
     else:
          new = scores[index]
     # 아래 두 줄은 while의 것이기때문에 줄 맞추지 않으면 안됨 에러도 안나옴 직접찾아야됨
     new_scores.append(new) 
     index = index + 1
print('71:', new_scores)
...
#     print('현재 사용량:' + time)
     ...
     if time >= 300:
          print('[사용중단] 하루 사용 권장량에 도달 또는 초과했습니다.')
          break
     else:
          time += 50
